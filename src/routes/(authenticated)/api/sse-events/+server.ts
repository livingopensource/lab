        ...
        .watch(
            ...
            (type, apiObj, watchObj) => {
                if (type === "ADDED") {
                    ...
                    const {error} = emit('message', `Bookmark ${watchObj.metadata.resourceVersion}`)
                    if (error) {
                        console.log(error)
                        return
                    }
                    ...
                    const {error} = emit('message', `Unknown response ${type}}`)
                    if (error) {
                        console.log(error)
                        return
                    }
                ...
                const {error} = emit('message', `Unknown generic response ${apiObj}}`)
                if (error) {
                    console.log(error)
                    return
                }
            ...
            err => {
                ...
                const {error} = emit('message', `Got an error ${err.message}}`)
                if (error) {
                    console.log(error)
                    return
                }
